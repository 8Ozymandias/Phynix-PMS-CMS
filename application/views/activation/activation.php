...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	if(!isset($_GET['secretHash']) || !isset($_GET['uniquekey']) || !isset($_GET['error'])){
		...
		if(strlen($_GET['secretHash']) < 50){
			redirect(site_url("Home")."?error=Error:invalid url");
		}else{

			if($this->ses->has_userdata("user_session")){
				redirect(site_url("Home")."?error=Account is active please login to continue");
			}else{
				 $this->load->helper('loads');
				$bool = checkSession("system_user",$_GET['uniquekey']);
				$cool = checkSession1("system_user",$_GET['uniquekey']);
				$error = $_GET['error'];
				if($bool == false || $cool == false){
					redirect(site_url("Home")."?error=Error:page not found, link expired");
				}else{

					if(($cool['user_hash'] == "" && $cool['manager_password'] != "") || $cool['manager_password'] != "" || $cool['user_hash'] == ""){
						redirect(site_url("Home")."?error=Error:page not found, link expired");
					}else{



					$username = $cool['manager_fname'].' '.$cool['manager_lname'];
					$date = $cool['date_joined'];

					$hash = $_GET['secretHash'];
					$key = $_GET['uniquekey'];

					$arrySes = array("hash"=>"{$hash}","key"=>"{$key}");
  ...
  <head>
    ...
    <script>
    	...
    	if(pass !== conp){
    		...
    		$.post("<?php echo site_url('Home/UdpRegUser'); ?>",{pass:pass,conp:conp},function(result){
    			setTimeout(function(){
    			$("#result").html(result);
    		},2000);
    		});
