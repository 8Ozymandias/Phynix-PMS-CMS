...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	if(!isset($_GET['secretHash']) || !isset($_GET['uniquekey']) || !isset($_GET['error'])){
		...
		if(strlen($_GET['secretHash']) < 100 || strlen($_GET['uniquekey']) < 70){
			...
			if($this->ses->has_userdata("user_session")){
				...
				$bool = checkSession3("property_lanlord",'landlord_session_id',$_GET['uniquekey']);
				$cool = checkSession4("property_lanlord",'landlord_session_id',$_GET['uniquekey']);
				$error = $_GET['error'];
				if($bool == false || $cool == false){
					redirect(site_url("Home")."?error=Error:page not found, link expired");
				}else{

					if(($cool['secret_hash'] == "" && $cool['landlord_password'] != "") || $cool['landlord_password'] != "" || $cool['secret_hash'] == ""){
						redirect(site_url("Home")."?error=Error:page not found, link expired");
					}else{



					$username = $cool['landlord_fname'].' '.$cool['landlord_lname'];
					$date = $cool['date_joined'];

					$hash = $_GET['secretHash'];
					$key = $_GET['uniquekey'];

					$arrySes = array("hashes"=>"{$hash}","keyes"=>"{$key}");
  ...
  <head>
    ...
    <script>
    	...
    	if(pass !== conp){
    		...
    		$.post("<?php echo site_url('Home/UdpRegUser1'); ?>",{pass:pass,conp:conp},function(result){
    			setTimeout(function(){
    			$("#result").html(result);
    		},1);
    		});
